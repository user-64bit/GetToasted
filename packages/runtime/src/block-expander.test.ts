...
import { detectSandwichesInSlot } from "@get-toasted/core";
import type {
  HeliusBlock,
...
describe("createBlockExpander — end-to-end against a synthetic sandwich block", () => {
  it("returns front+victim+back swaps in true block order, and the detector finds the sandwich", async () => {
    ...
    // Now feed those swaps into the real detector — proves the new pipeline
    // actually surfaces a sandwich. This is the assertion the user cares
    // about: "with my fix, does the system find sandwiches?" Yes.
    const detections = detectSandwichesInSlot(swaps, null);
    expect(detections).toHaveLength(1);
    expect(detections[0]!.candidate.victim.signer).toBe(VICTIM);
    expect(detections[0]!.candidate.front.signer).toBe(ATTACKER);
    expect(detections[0]!.candidate.back.signer).toBe(ATTACKER);
    expect(detections[0]!.attackerProfitRaw).toBe(100_000_000n);
  });
